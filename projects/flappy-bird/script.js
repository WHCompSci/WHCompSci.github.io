...
const font = new FontFace(
    "Silkscreen",
    "url(https://fonts.gstatic.com/s/silkscreen/v4/m8JXjfVPf62XiF7kO-i9YLNlaw.woff2)"
)
birdImg.onload = () => {
...
function isColliding() {
    for (let i = 0; i < pipes.length; i++) {
        const pipe = pipes[i]
        const onPipe =
            bird.x + bird.r > pipe.x && bird.x - bird.r < pipe.x + thickness
        if (!onPipe) continue
        const onFrontEdge = bird.x + bird.r > pipe.x && bird.x - bird.r < pipe.x
        if (bird.y - bird.r < pipe.y - gap) {
            return {
                collisionType: onFrontEdge ? "front" : "top",
                pipeIndex: i,
            }
        }
        if (bird.y + bird.r > pipe.y + gap) {
            return {
                collisionType: onFrontEdge ? "front" : "bottom",
                pipeIndex: i,
            }
        }
